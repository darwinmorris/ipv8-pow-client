...
AHEAD_BY = 5
TEST_DIFFICULTY = 8
# These adoption/fork tests build short, fixed-difficulty chains. Align the
# bootstrap difficulty with them so they sit inside the retarget's bootstrap
# window and the adaptive schedule check accepts them. (The retarget itself is
...
def test_retry_missing_block_keeps_lower_fork_height_inside_reorg_window():
    _keys, _private_keys, (community, _peer, _other) = make_miners()
    install_chain(community, mine_chain(9))  # local height 8

    requested = []
    community.request_block = lambda height, peer=None: requested.append((height, peer))  # type: ignore[method-assign]

    community.missing_block_requests[3] = (0.0, 1)

    with patch("src.community.time.time", return_value=10.0):
        community.retry_missing_blocks()

    assert community.missing_block_requests[3][1] == 2
    assert requested == [(3, None)]
...
def test_branch_waits_for_missing_transaction_in_parent_block():
    ...
    # Parent arrives, but its transaction body is not known yet.
    accepted, missing, missing_txs = bc.add_block(b1)
    assert not accepted
    assert missing is None
    assert missing_txs == [tx.tx_hash]

    # Retrying adoption must still refuse the branch while b1's tx is unavailable.
    bc.try_adopt()
    assert bc.height == 0
    ...
    assert bc.chain[1].block_hash == b1.block_hash
    assert bc.tip.block_hash == b2.block_hash
    assert tx.tx_hash in bc.chain_tx_hashes
